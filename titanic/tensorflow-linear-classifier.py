...
model_dir = tempfile.NamedTemporaryFile(delete=True).name #"./data/model"
...
def build_estimator():
  ...
  columns = [name, gender, pclass, embarked, cabin, age, age_buckets, siblings, parents, fare]
...
'''
train, train_test = read_data_and_return_train_test(split=0) # 20% test, 80% train

# fit the model against the train set
model = build_estimator()
print("Training on train data:"+ str(train.shape))
model.fit(input_fn=lambda: input_fn(train), steps=200)

# evaluate model against the 20% of the train data “train_test”
results = model.evaluate(input_fn=lambda: input_fn(train_test), steps=1)
...
predictions = enumerate(model.predict(input_fn = lambda:input_fn2(test)))
